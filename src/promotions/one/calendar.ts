...
function descriptionFor(event: OneEvent): CalendarDescriptionModel {
  ...
  const bouts: CalendarBoutModel[] = event.bouts.map((bout, index) => ({
    ...
    red: {
      ...
      facts: fighterFacts(bout.redRecord, bout.redAge, formatPromotionOdds(bout.redOdds), bout.redStyle),
    },
    blue: {
      name: bout.blueName,
      shortName: shortPromotionFighterName(bout.blueName),
      flag: countryFlags(bout.blueCountry),
      facts: fighterFacts(bout.blueRecord, bout.blueAge, formatPromotionOdds(bout.blueOdds), bout.blueStyle),
    },
