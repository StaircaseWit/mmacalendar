...
</style></head><body><h1>MMA odds history</h1><p>Odds are snapshotted no more than once every seven days. A row is added only for the initial value or a change.</p><p>Last checked: ${htmlEscape(checked)}</p><table><thead><tr><th>Event / fight</th><th>Checked</th><th>Entry</th><th>Odds</th></tr></thead><tbody>${rows || "<tr><td colspan=\"4\">No odds have been recorded yet.</td></tr>"}</tbody></table></body></html>\n`;
}
